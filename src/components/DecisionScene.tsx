...
import { careerDecisionIdentity, choiceArchetype, eventStakeStats, presentedRiskLabel, riskLevel, sceneProfile, STAT_LABELS } from '../game/experience'
import { presentChoiceHint, presentChoiceText, presentEventDescription, presentEventTitle } from '../game/presentation'
...
export function DecisionScene({ event, player, onChoose }: { event: CareerEvent; player: CareerPlayer; onChoose: (choice: EventChoice) => void }) {
  ...
  const maximumRisk = Math.max(...event.choices.map((choice) => riskLevel(presentedRiskLabel(choice))))
  ...
  return <section className={`decision-scene scene-${profile.id}`} aria-labelledby="decision-title">
    ...
    <div className="scene-layout">
      ...
      <aside className="scene-sideline">
        ...
        <div className="scene-pulse"><span>TU PULSO</span><Pulse label="Mentalidad" value={player.stats.confidence} /><Pulse label="Velocidad" value={player.stats.fitness} /><Pulse label="Resistencia" value={player.stats.resilience} /></div>
    ...
    <div className="decision-choices">{event.choices.map((choice, index) => {
      ...
      const riskLabel = presentedRiskLabel(choice)
      const level = riskLevel(riskLabel)
      return <button type="button" className={`decision-choice choice-${archetype.id}`} aria-keyshortcuts={String(index + 1)} key={choice.id} onClick={() => onChoose(choice)}>
        ...
        <p>{presentChoiceHint(choice.visibleHint)}</p>
        ...
        <div className="choice-risk"><span>{riskLabel === 'Resultado impredecible' ? 'IMPREDECIBLE' : riskLabel.toUpperCase()}</span><i>{[1, 2, 3, 4, 5].map((risk) => <b className={risk <= level ? 'active' : ''} key={risk} />)}</i></div>
      </button>
    })}</div>
  </section>
}
